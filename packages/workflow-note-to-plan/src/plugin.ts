...
export const configSchema: ConfigSchema = {
  ...
  baseBranch: {
    type: "record",
    description:
      "where one repository's base branch is, instead of the fallback. A repository named here has its plan opened against that branch",
    default: {},
  },
  policy: {
    type: "record",
  ...
  register(registry, ctx) {
    ...
    registry.contribute(WORKFLOW_RUNTIME, noteToPlan.name, {
      ...
      apply: (
        ...
        now: Date,
        moved: Moved | null,
      ): PlanRecord => lazily().apply(record, plan, outcomes, observation, now, moved),
