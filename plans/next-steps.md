...
| 11 | [An update keeps the workflow current](an-update-keeps-the-workflow-current.md) | Every workflow runs with a persisted update cadence; the default is enabled before every twentieth run, and an operator can disable it or put it after the work without a daemon replacing itself underneath its loaded modules. |
| 13 | [A skill ladder for your own steps](a-skill-ladder-for-your-own-steps.md) | A workflow somebody wrote gives one of its own steps a cheaper model and a skill, from `config.yaml` |
...
| 14 | [Work that is over goes away](work-that-is-over-goes-away.md) | A record that ended leaves the disk on its own, and one piece of work that will never finish on its own can be retired with a command rather than with `rm` |
...
| 25 | [The errand is proven end to end](the-errand-is-proven-end-to-end.md) | Editing the errand workflow or the task store turns `sf check` red until the errand scenario is run again and resealed, the same way the other two shipped workflows already behave |
