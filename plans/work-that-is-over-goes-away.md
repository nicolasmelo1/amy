...
[Finished work leaves the listing](finished-work-leaves-the-listing.md) took
the other cheap half: `amy status` no longer prints what has ended. That hides
the record. It does not remove it, and a store that never removes anything is
still a store that grows for as long as the machine runs.
