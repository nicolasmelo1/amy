...
describe("LinearTracker.get", () => {
  ...
  // `issue(id:)` is `Issue!` in Linear's schema, so a gone ticket is never
  // `null`: it is the not-found error, which is the answer "nothing".
  ...
  it("does not read any other failure as a gone ticket", async () => {
    ...
    await expect(new LinearTracker(client, config).get("PROJ-9999")).rejects.toThrow(/Rate limit exceeded/);
  });
});
